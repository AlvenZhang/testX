...
|---------|--------|---------|---------|---------|
| **移动端-A** | iOS测试无法在Linux Docker中运行（Apple限制） | 🔴 高 | 100% | 使用macOS物理机或云macOS服务 |
| **移动端-B** | Android模拟器在Docker中无法运行（需要x86虚拟化） | 🔴 高 | 100% | 使用真机 + adb connect网络连接 |
| **移动端-C** | 不同Android版本的Appium兼容性差异 | 🟡 中 | 60% | 限制支持版本（Android 10-13） |
| **移动端-D** | 真机设备池管理复杂度高 | 🟡 中 | 40% | 分阶段实现，先支持模拟器 |
| **AI能力-A** | AI生成的代码可运行率低于预期 | 🟡 中 | 50% | 保留人工编辑功能作为兜底 |
| **AI能力-B** | AI影响分析准确率不足 | 🟡 中 | 50% | 提供手动确认环节 |
| **AI能力-C** | 多模型切换后输出格式不一致 | 🟡 中 | 30% | 统一后处理解析逻辑 |
| **架构-A** | Docker沙箱中无法访问宿主机USB设备 | 🔴 高 | 100% | 改用网络连接的真机设备 |
| **架构-B** | WebSocket高并发下内存泄漏 | 🟡 中 | 30% | 使用Redis Pub/Sub中转 |
| **集成-A** | Git API（GitHub/GitLab/Gitee）接口差异 | 🟡 中 | 40% | 适配器模式封装 |

### 5.2 技术验证矩阵（带备选方案）

| 序号 | 技术点 | 验证目标 | 验证方法 | 通过标准 | 备选方案（验证失败时） |
|------|--------|----------|----------|---------|----------------------|
| **V1** | **FastAPI异步性能** | 单机QPS满足需求 | 压测100并发CRUD | QPS > 500 | 使用同步框架 + Gunicorn多进程 |
| **V2** | **MySQL JSON字段** | 存储和查询config数据 | 存储10万条，索引查询 | 查询 < 100ms | 拆分JSON为独立字段 |
| **V3** | **Redis缓存** | AI响应缓存命中 | 相同输入重复请求 | 缓存命中 | 降级到内存缓存 |
| **V4** | **Docker沙箱-Web** | Web/接口自动化执行 | 创建+执行+销毁循环 | < 30秒 | 使用本地进程池（降低隔离性） |
| **V5** | **Android真机连接** | ADB网络连接真机并截图 | adb connect + screencap | 截图成功 | 使用USB直连（需物理接触） |
| **V6** | **Appium Android** | 执行简单测试用例 | 启动Appium，点击按钮，截图 | 测试通过，截图成功 | 限制Android版本范围 |
| **V7** | **Appium iOS** | iOS真机/模拟器测试 | 启动WDA，执行XCUITest | 测试通过，截图成功 | **阶段一先不支持iOS** |
| **V8** | **AI用例生成** | 生成可执行的测试用例 | 输入10个需求，评估质量 | 可运行率 > 70% | 提供用例模板，人工微调 |
| **V9** | **AI影响分析** | 识别受影响的旧用例 | 修改5个需求后测试 | 召回率 > 60% | 降低自动化程度，辅助分析 |
| **V10** | **AI代码修复** | 修复失效测试代码 | 修改5个需求后修复代码 | 可运行率 > 70% | 保留人工编辑，AI仅提供建议 |
| **V11** | **AI模型稳定性** | API调用成功率 | 连续100次调用 | 成功率 > 99% | 多模型自动切换 |
| **V12** | **WebSocket推送** | 实时日志延迟 | 执行时测量延迟 | < 1秒 | 降级到轮询 |
| **V13** | **Git对接** | 拉取代码和解析diff | 对接GitHub/GitLab/Gitee | diff解析正确 | 简化为手动上传diff |
| **V14** | **多项目隔离** | 数据完全隔离 | 跨项目查询 | 无数据泄露 | 强制schema隔离 |
| **V15** | **报告导出** | 生成PDF/HTML报告 | 导出并验证 | 文件可打开 | 仅支持HTML截图导出 |
...
**Day 1-2: 环境搭建**
```
1. Docker + MySQL + Redis + FastAPI 基础环境
2. 验证异步数据库操作（V1, V2, V3）
3. 输出：环境部署文档、基础API性能报告
```

**Day 3-4: 移动端技术验证 🔴 核心风险点**
```
1. Android真机测试验证（V5, V6）
   - 准备：一台Android真机（开启USB调试）
   - 验证：adb connect网络连接 → Appium连接 → 简单测试执行
   - 关键问题：Docker容器内能否通过网络控制真机？

2. iOS测试验证（V7）
   - 准备：macOS机器（必须）
   - 验证：XCRun + WDA + XCUITest
   - 关键问题：是否需要单独macOS服务器？
```
...
**Day 6-7: AI能力验证（V8, V9, V10, V11）**
```
1. DeepSeek API接入测试
2. GPT-4o API接入测试（需要代理）
3. 用例生成质量评估
4. 影响分析准确率评估
5. 代码修复可运行率评估
```

**Day 8-9: 集成验证（V12, V13, V15）**
```
1. WebSocket实时推送测试
2. Git对接测试（GitHub/GitLab/Gitee）
3. 报告导出测试
```
